...
uv run manga_dosei YYYYMMDD --session-id YYYYMMDD_retry1 # distinct .adk session keyed by the suffix
                                                         # session-id must match ^\d{8}(_[A-Za-z0-9_-]+)?$
                                                         # and its first 8 chars must equal target_date
...
# Settings, which prefers the shell env and falls back to a CWD-relative `.env`.
GITHUB_OUTPUT_TOKEN=... \
...
Publishing is a **separate, non-LLM CLI** in `manga_dosei/publish.py`. It takes `--source` (typically the `--publish-dir` output) plus `--repo / --dest / --message / --branch` and pushes everything as one fast-forward commit via PyGithub's Git Data API (blob → tree → commit → ref.edit). Token comes from `GITHUB_OUTPUT_TOKEN` read inline via `Settings(_env_file=None).github_output_token` — **process env only**, never from CLI args and never from a `.env` file. (The pipeline's `get_settings()` does read a CWD-relative `.env`; the publisher deliberately constructs `Settings(_env_file=None)` so a stray `.env` in an arbitrary CI CWD — a publish-dir mounting a dev home, a CI cache reused across projects, a developer invoking from a sibling project — cannot inject `GITHUB_OUTPUT_TOKEN`.) The pipeline and the publisher are intentionally separate processes so the LLM-driven pipeline never holds the token in memory. The typical CI pattern is: `exec` stdout/stderr to files inside the same `--publish-dir`, run the pipeline, then run `manga_dosei-publish` so the captured logs ride the same commit.
...
- `inspect_artifacts` takes a `target_date` argument that is **format-validated only and otherwise informational** (echoed back into the result payload). The actual artifact / state lookup uses the current `ToolContext` / session, not `target_date`, so passing the wrong date does not change what is returned.
