...
* **台本に書かれていない文字を画像内に描かない**:
  台本にないキャプション、横断幕、看板の説明文、字幕、キャラ名表示、
  日付ラベル、解説テキストなどを背景や前景に追加しないでください。
  背景文字や小道具の文字は、**台本の「イラスト:」欄または
  レイアウトの「視覚要素」欄に明記されたものに限定**します。
  あるコマの背景文字を別のコマの背景に流出させないでください。
* **描画してはいけない台本上の記法**:
