...
_SAMPLES_DIR = (
    Path(__file__).resolve().parent.parent.parent / "assets" / "samples"
)
...
* **ヘッダ（日付とタイトル）**: ページ上部のヘッダは**2行構成**で配置してください。
  * 1行目: 日付のみ（例: `2026年04月18日`）。タイトルより小さめの黒色太字。
  * 2行目: タイトル本文（例: `静養！公邸でじっくり英気を養う土曜日や！`）。日付より**明確に大きい**黒色太字。
  * 台本の「タイトル」欄は `YYYY年MM月DD日 <タイトル本文>` の形式で記載されているので、先頭の日付部分を1行目、それ以降を2行目に分けて描画してください。
  * 改行位置・フォント・サイズ比・配置（左寄せ/中央など）・行間・余白は添付された【ページ例】に忠実に従ってください。
* 漫画のコマは「右から左へ」、そして「上から下へ」読みます。読みやすいコマ割りを意識してください。
* 各コマの左上には「コマタイトル」を記載します。台本に書かれたコマタイトルを、`#F0E68C` の背景色、黒色の太字ゴシック体で、そのまま正確に描画してください。詳細なフォーマット（フォント、サイズ、配置など）は添付された「ページ例」のスタイルに揃えてください（内容は台本に従ってください）。
* 「漫画のコマ割り（パネルレイアウト）のルール」を遵守してください。
* ウォーターマークは入れないでください
* **台本の文章を一字一句そのまま使用**: 台本に記載されているコマタイトル、セリフ、ナレーション、擬音、説明文などの文章は、一切変更・創作せずにそのまま正確に描画してください。漢字・ひらがな・数字を含め、誤字や字形の間違いは避けてください。
* **描画してはいけない台本上の記法**: 台本のセリフは「`1. 高市総理「...セリフ本文...」`」のような形式で書かれています。**吹き出しに描画するのは「」内の本文のみ**です。以下は**描画しないでください**:
  * 先頭の番号プレフィックス（`1.`、`2.`、`3.` など）— これはコマ内でのセリフの読み順を示すマーカーであり、吹き出しに含めてはいけません。
  * 話者ラベル（`高市総理`、`小泉防衛相`、`マールズ副首相` など「」の前に書かれた名前）— これは発言者を指定するためのメタ情報であり、吹き出しに描画する文字ではありません。話者ラベルに従って、**吹き出しのしっぽを該当キャラクターへ向けてください**（「吹き出し配置ルール」参照）。
  * `（心の声）`、`（資料を読みながら）` などの補助注釈は、必要に応じて吹き出しの形（思考バルーン等）や表情・仕草で表現し、文字として吹き出し内に描画する必要はありません。
* **キャラクターリファレンス**: 添付された「キャラクターリファレンス」画像を参照して、登場人物の顔や外見を統一してください。台本に登場する各キャラクターについて、対応するリファレンス画像がある場合は、すべてのコマで同じキャラクターデザインを使用してください。表情やポーズは変えても構いませんが、顔の特徴（目、鼻、口、髪型など）はリファレンス画像に忠実に再現してください。複数のキャラクターリファレンスが添付されている場合は、それぞれのキャラクターを適切に識別して使用してください。
* 「吹き出し（セリフ）の配置ルール」を遵守してください
* **画風**: 日本のフラットなアニメ/セル画調で描画してください。均一なベタ塗りを基本とし、立体的なグラデーション・強いハイライト・髪や肌のツヤ・写実的なライティングや影は使わないでください。背景もアニメ背景として描画し、写真のようなディテールや質感を入れないでください。全体のトーンは添付された【ページ例】に忠実に揃えてください（Pixar 風・3D CG 風・セミリアル調にはしない）。
* **文字要素（フォント・サイズ・色）**: タイトル、コマタイトル、セリフ、ナレーション、擬音、説明文、免責事項など**すべての文字要素**について、**フォント（書体）・サイズ・色・太さ・装飾**を【ページ例】に忠実に揃えてください。独自の装飾的フォントや派手な色は使わず、【ページ例】で使用されている書体・配色のルールを厳密に模倣してください。
...
* 上記の図はあくまで「読み順のルール」の参考です。実際のコマ数や配置は、シーンの内容に最適なものを生成してください。
...
**発言者は台本の話者ラベルで決まります**。台本のセリフは「`<番号>. <話者ラベル>「...本文...」`」形式で記載されており、`<話者ラベル>` が発言者です。例えば「`1. 小泉防衛相「自由で開かれた…」`」の発言者は小泉防衛相であり、吹き出しのしっぽは小泉防衛相へ向けてください。マールズ副首相のセリフを小泉防衛相に付けるような**発言者の取り違えは厳禁**です。
...
* **【ページ例】**: レイアウト、フォーマット、スタイル（タイトルの書式、コマタイトルの
  ...
  **内容を同じにする必要はありません**が、**画風・質感・トーン、および文字要素のフォント・サイズ・色・配置ルールはこの画像を忠実に模倣してください**。
  ...
  写実的・立体的・3D CG 的な質感に寄せないでください。台本の内容に基づいて描画してください。
...
async def generate_page_gemini(
    ...
    if page_number not in _PAGE_NUMBER_RANGE:
        return _error(
            f"page_number must be in {_PAGE_NUMBER_RANGE.start}..{_PAGE_NUMBER_RANGE.stop - 1}, "
            f"got {page_number}",
    ...
    asset_keys = sorted(
        key
        for key in await tool_context.list_artifacts()
...
def _error(message: str, *, page_number: Optional[int] = None) -> dict[str, Any]:
