...
* **描画してはいけない台本上の記法**:
  台本のセリフは「`1. <話者名>「...セリフ本文...」`」のような形式で書かれています。
...
1. すべての複数人コマで、上の「🔴 描画前チェックリスト」の左右位置を満たしているか。
2. 各吹き出しのテールが、レイアウト指示書で指定された発言者の口元へ向いているか。
...
- 「キャラ配置」フィールドの `画面左=` / `左中=` / `中央=` / `右中=` / `画面右=` は
  **コマ内の物理的な左右位置** を指定する。画像の左側＝画面左、画像の右側＝画面右。
- 例: `画面左=A、中央=B、画面右=C` なら、
  コマの **左側に A、中央に B、右側に C** を必ず描画する。
- この配置は吹き出しの読み順 (右→左 = #1→#N の発言順) と整合させるための制約であり、
  入れ替えると発言者と吹き出しが一致しなくなり成立しない。
...
async def generate_page_gpt(
    ...
    # layout 指示書は **画像生成時に最後に読まれる** よう末尾に配置する
    # （モデルは末尾の指示を強く反映する傾向があり、配置バイアスを抑える）。
    prompt = (
        f"{_PROMPT_TEMPLATE}{scenario_text}\n```"
        f"{_IMAGE_ORDER_TEMPLATE.format(lines=order_lines)}"
        f"\n\n---\n\n{_LAYOUT_HEADER}{layout_text}\n\n{_FINAL_REMINDER}"
    )
